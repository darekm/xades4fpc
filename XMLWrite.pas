...
type
  ...
  protected
    ...
    procedure VisitAttribute(Node: TDOMNode;hasIndent:boolean=false);
...
begin
  ...
  if node.HasAttributes then begin
    attributesHasIndent:= node.Attributes.Length>10;
    for i := 0 to node.Attributes.Length - 1 do
    begin
      child := node.Attributes.Item[i];
      if TDOMAttr(child).Specified then
        VisitAttribute(child,AttributesHasIndent);
    end;
  end;
...
procedure TXMLWriter.VisitAttribute(Node: TDOMNode;hasIndent:boolean=false);
var
  Child: TDOMNode;
begin
  if hasIndent then
