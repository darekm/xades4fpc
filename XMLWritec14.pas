...
var
  ...
  function  sortNameNS(aNode : tDomNode):string;
  begin
        result:=aNode.nodeName;
        if (pos(':',result)=0) and (pos('xmlns',result)>0)  then
          result:=result+'!';
        result:=result + '="'+aNode.nodeValue + '"';
  end;
  ...
  begin
      ...
      begin
        ...
        begin
          xNSName:=sortNameNS(aNode.attributes.item[i]);
...
begin
  ...
  try
    ...
    while (element.ParentNode <>nil) and (element.parentNode is tDomElement) do begin
    ...
    begin
      xNSName:=sortNameNS(element.attributes.item[i]);
