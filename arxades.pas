...
type
  ...
  public
    ...
    procedure loadStr(const fStr: string);
  ...
  public
    ...
    function digestValue(aNode: tDomElement): xadesstring;
...
begin
  ...
  sn := '<ds:Object xmlns:ds="http://www.w3.org/2000/09/xmldsig#" Id="Dokument-0" MimeType="text/xml">'
    + '<test>'#10 + '<t>a</t>'#10 + '</test>' + '</ds:Object>';
  s6 := 'kEaE9Pupi3N/yk/Lkh7G5z4h2Vg=';
  Result := StrtoBase64(sha1(sn));
  if Result <> s6 then
    Result := 'ss';
  sn := '<ds:Object xmlns:ds="http://www.w3.org/2000/09/xmldsig#" Id="Dokument-0" MimeType="text/xml"><test>'#10 + '<b><t>a</t></b>'#10 + '</test></ds:Object>';
  s6 := 'S/LzzR1DODJNnOHCy9nZoHNqJMg=';
  Result := StrtoBase64(sha1(sn));
  if Result <> s6 then
    Result := 'ss';

  sn := '<ds:Object xmlns:ds="http://www.w3.org/2000/09/xmldsig#" Id="Dokument-0" MimeType="text/xml"><test>'#10 + '<b>'#10 + '<t>a</t></b>'#10 + '</test></ds:Object>';
  ...
  sn := '<ds:Object xmlns:ds="http://www.w3.org/2000/09/xmldsig#" Id="Object1_0c3128b2-97d4-4d6f-b9b1-9c485142c051" MimeType="text/xml"><test>'#10 + '<b>'#10 + '   <t>a</t></b>'#10 + '</test></ds:Object>';
...
begin
  ...
  sn := sn + 'AAOBjQAwgYkCgYEApW8rqS/u8ySAJDOTEEAfeOrCNeTGBCvu/QXDYu2QwPbAaOs7' +
    ...
    'J0YA5QGQkRVhcg6pVefqNp7CsBmAEp7GalmdTCKboARmt+arWwPcbLMkeLHQCtBp' + 'tnUPfKJuxTZW+5kLWQ==';
  ...
  Result := StrtoBase64(sha1(Base64tostr(sn)));
...
//   result:='some text';
   ...
   sn:=Base64tostr('7/XTsHaBSOnJ/jXD5v0zL6VKYsk=');
   if sn<>s6 then
   result:=result+' ';
 }
...
{  with TSHA1.Create(Pointer(Msg)^,Length(Msg)) do begin
  ...
  if sn = HexToOS('a9993e36 4706816a ba3e2571 7850c26c 9cd0d89d') then
  begin
    //    writeln(sn);
  end;
  {
...
begin
  Result := StrtoBase64(digestValue(aNode));
end;
...
var
  sTIme: string;
  xData, xElement, xQualify, xSigned: tDomElement;

begin
  xData := fXades.doc.createElement('xades:SignedSignatureProperties');
  xElement := fXades.doc.createElement('xades:SigningTime');
  sTime := '2012-02-25T15:26:46+01:00';
  //  '2012-11-15T13:58:26.167+09:00';
  //  '2009-08-12T16:32:27Z';
  xElement.appendChild(fXades.doc.createtextNode(sTime));
...
var
  xData, xCertificate: tDomElement;
begin
